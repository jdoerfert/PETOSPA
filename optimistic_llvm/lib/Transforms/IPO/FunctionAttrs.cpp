...
struct ArgumentUsesTracker : public CaptureTracker {
  ...
  void tooManyUses() override {
    //errs() << "captured by due many uses\n";
    Captured = true; }

  bool captured(const Use *U) override {
    CallSite CS(U->getUser());
    if (!CS.getInstruction()) {
      //errs() << "captured by use: " << *U << " : " << *U->getUser() << "\n";
      Captured = true;
      return true;
    }
    ...
    if (!F || !F->hasExactDefinition() || !SCCNodes.count(F)) {
        //errs() << "captured by cs of " << F->getName() << " "
               //<< F->hasExactDefinition() << "\n";
      if (CS.isArgOperand(U))
        //errs() << "ArgNo: " << CS.getArgumentNo(U) << " : "
               //<< *CS.getArgument(CS.getArgumentNo(U)) << "\n";
      Captured = true;
      return true;
    }
    ...
    if (UseIndex >= CS.getNumArgOperands()) {
      ...
      // conservative.
      //errs() << "captured by due to use in op bundles in " << F->getName()
             //<< "\n";
      Captured = true;
      return true;
    }
    ...
    if (UseIndex >= F->arg_size()) {
      ...
      //errs() << "captured by due to use in var args in " << F->getName()
             //<< "\n";
      Captured = true;
      return true;
    }
...
static bool addArgumentAttrs(const SCCNodeSet &SCCNodes) {
  ...
  //errs() << "SCC:\n";
  //for (Function *F : SCCNodes)
    //errs() << " - " << F->getName() << " [E: " << F->hasExactDefinition()
           //<< "]\n";
  ...
  for (Function *F : SCCNodes) {
    ...
    for (Function::arg_iterator A = F->arg_begin(), E = F->arg_end(); A != E;
      ...
      if (!A->hasNoCaptureAttr()) {
        ...
        //errs() << "Arg: " << *A << " "
               //<< (Tracker.Captured ? "Captured!" : "not captured!")
               //<< " #U: " << Tracker.Uses.size() << "\n";
        if (!Tracker.Captured) {
  ...
  for (scc_iterator<ArgumentGraph *> I = scc_begin(&AG); !I.isAtEnd(); ++I) {
    ...
    //errs() << "AGSCC:\n";
    //for (auto *AGN : ArgumentSCC) {
      //if (AGN->Definition)
        //errs() << " - " << *AGN->Definition << " @ "
               //<< AGN->Definition->getParent()->getName() << "\n";
      //else
        //errs() << " - ROOT \n";
      //for (auto *U : AGN->Uses)
        //errs() << "   -- " << *U->Definition << " @ "
               //<< U->Definition->getParent()->getName() << "\n";
    //}
    ...
    for (auto I = ArgumentSCC.begin(), E = ArgumentSCC.end();
      ...
      if (Node->Uses.empty()) {
        if (!Node->Definition->hasNoCaptureAttr()) {
          //errs() << "Capture by " << *Node->Definition << "\n";
          SCCCaptured = true;
        }
      }
    }
    //errs() << "SCC Captures " << SCCCaptured << "\n";
    if (SCCCaptured)
      continue;
